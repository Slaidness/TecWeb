...
function validate ( )
{
    ...
    if ( !USERNAME.val() || USERNAME.val().length < 5  )
    {
        ...
        USERNAME_MSG.html( "Username deve contenere almeno 5 lettere o piu`" );
        USERNAME_MSG.show();
    ...
    {
        PASSWORD_MSG.html("La password deve avere una lunghezza minima di 8 caratteri");
        PASSWORD_MSG.show();
        valid = false;
    }
    ...
    {
        CONFIRM_MSG.html("Le passwords non combaciano");
        CONFIRM_MSG.show();
        valid = false;
    }
    ...
    if ( atpos < 1 || dotpos < atpos + 2 || dotpos + 2 >= x.length ) {
        EMAIL_MSG.html("Inserisci email valida");
        EMAIL_MSG.show();
        valid = false;
    }
